...
var ImageViewer = /** @class */ (function (_super) {
     ...
     * props 有变化时执行
     */
    ImageViewer.prototype.reInit = function (nextProps) {
        var _this = this;
        ...
        // 给 imageSizes 塞入空数组
        var imageSizes = [];
        nextProps.imageUrls.forEach(function (imageUrl, index) {
            var status = _this.loadedIndex.has(index) ? 'success' : 'loading';
            imageSizes.push({
                width: imageUrl.width || 0,
                height: imageUrl.height || 0,
                status: status,
